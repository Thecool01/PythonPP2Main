...
def spawn_apple():
    while True:
        ...
        if (new_apple not in snake_body and new_apple not in fruits and 
            new_apple not in walls):
            return new_apple
...
def spawn_orange():
    while True:
        new_orange = [random.randrange(1, game_width // cell_size - 1) * cell_size + game_offset_x,
                      random.randrange(1, game_height // cell_size - 1) * cell_size + game_offset_y]
        if (new_orange not in snake_body and new_orange not in fruits and 
            new_orange not in walls):
            return new_orange
...
def spawn_banana():
    while True:
        new_banana = [random.randrange(1, game_width // cell_size - 1) * cell_size + game_offset_x,
                      random.randrange(1, game_height // cell_size - 1) * cell_size + game_offset_y]
        if (new_banana not in snake_body and new_banana not in fruits and 
            new_banana not in walls):
            return new_banana
...
def spawn_time_fruit():
    while True:
        new_time_fruit = [random.randrange(1, game_width // cell_size - 1) * cell_size + game_offset_x,
                          random.randrange(1, game_height // cell_size - 1) * cell_size + game_offset_y]
        if (new_time_fruit not in snake_body and new_time_fruit not in fruits and 
            new_time_fruit not in walls):
...
def create_walls_for_level(level):
    ...
    if level == 1:
        ...
        # Simple border walls
        for x in range(game_offset_x, game_width + game_offset_x, cell_size):
            walls.append([x, game_offset_y])
            walls.append([x, game_height + game_offset_y - cell_size])
        for y in range(game_offset_y, game_height + game_offset_y, cell_size):
            walls.append([game_offset_x, y])
            walls.append([game_width + game_offset_x - cell_size, y])
    elif level == 3:
        # Cross in the middle
        center_x = game_width // 2 + game_offset_x
        center_y = game_height // 2 + game_offset_y
        for i in range(5):
            walls.append([center_x, center_y - 2*cell_size + i*cell_size])
            walls.append([center_x - 2*cell_size + i*cell_size, center_y])
    elif level == 4:
        # Four squares in corners
        for i in range(3):
            for j in range(3):
                # Top-left
                walls.append([game_offset_x + i*cell_size, game_offset_y + j*cell_size])
                # Top-right
                walls.append([game_width + game_offset_x - (i+1)*cell_size, game_offset_y + j*cell_size])
                # Bottom-left
                walls.append([game_offset_x + i*cell_size, game_height + game_offset_y - (j+1)*cell_size])
                # Bottom-right
                walls.append([game_width + game_offset_x - (i+1)*cell_size, game_height + game_offset_y - (j+1)*cell_size])
    elif level >= 5:
        # Maze-like pattern for higher levels
        for i in range(1, game_width // cell_size - 1, 3):
            for j in range(1, game_height // cell_size - 1, 3):
                if random.random() < 0.3:  # 30% chance to place a wall block
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_UP and direction != 'DOWN':
                ...
                banana = spawn_banana()
                time_fruit = spawn_time_fruit()
                fruits = [apple, orange, banana, time_fruit]
                fps = 5
                score = 0
                new_level_at = 10
                level = 1
    ...
    if alive:
        ...
        # Check collisions with borders, walls or self
        if (snake_pos[0] < game_offset_x or snake_pos[0] >= game_width + game_offset_x or
                ...
                snake_pos in snake_body or snake_pos in walls):
            alive = False
        else:
    ...
    if score >= new_level_at:
        ...
        new_level_at += 10
        fps += 0.5
        create_walls_for_level(level)  # Create new walls for the new level
